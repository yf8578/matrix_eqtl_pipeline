...
export PROJ_DIR='/data/work/01_analysis'
export CODE_DIR="/data/work/01_analysis/matrix_eqtl_pipeline"  # Path to the source code (src/)
export OUTPUT_DIR="$PROJ_DIR/1st_group/results_1st"


# Input Data
VCF_FILE="/data/input/Files/zhangyifan11/tianjin/result.ped_ann.vqsr.GTpost.mendelian.pass.Ex60MERR328.miss01AC1.dp10gq20.vaf20.iid.parent.sex.famQC.bi.lcr.clean.impute.lift2hg38.vcf.gz"
EXPRESSION_FILE="/data/work/00_data/TPM_lncRNA_1st_Trimester.txt"
GTF_FILE="/data/work/Ref/gencode.v46.annotation.gtf" # Optional if using fetch-pos
KNOWN_COV_FILE="/data/work/01_analysis/1st_group/1st_known_cov.csv"
...
if [ ! -f "$OUTPUT_DIR/gene_locations.txt" ]; then
    ...
    python3 "$CODE_DIR/src/matrix_eqtl/preprocessing/locations.py" \
        ...
        --format matrixeqtl \
        --fetch-pos --id-type "external_gene_name,ensembl_gene_id"
else
