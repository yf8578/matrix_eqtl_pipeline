...
## 🔍 手动分步执行指南 (Manual Step-by-Step Guide - MatrixEQTL)以eQTL和mQTL为例

```
# pQTL 数据初始输入
- 分子矩阵：/data/work/example/pQTL/input/expression.tsv
- vcf: /data/work/example/pQTL/input/genotypes.vcf.gz
- peer因子：(generated in Step 4b)
- 已知协变量：/data/work/example/pQTL/input/covariates.csv
...
MatrixEQTL计算cis和transQTL时，需要 `分子表型`和 `SNP`的位置文件，这里主要是转录组、蛋白组、蛋白磷酸化、甲基化这几个组学数据➡️分子在基因组位置 ( `gene_locations.txt`) 和 SNP 位置 (`snp_positions.txt`)。
...
```bash
# 这里对蛋白质构建位置文件，使用的id是uniprot_gn_id
python3 "$CODE_DIR/src/matrix_eqtl/preprocessing/locations.py" \
        --expression "$EXPRESSION_FILE" \
        --out "$OUTPUT_DIR/gene_locations.txt" \
        --format matrixeqtl \
        --fetch-pos --id-type "uniprot_gn_id"


# [Step 0] Checking Location Files...
# Generating gene locations...
# Reading Expression: /data/work/example/pQTL/input/expression.tsv
# --- Searching for 7696 IDs using type: uniprot_gn_id ---
# Querying BioMart for 7696 IDs (uniprot_gn_id)...
#   Found 7547 matches.
# Merging features...
#   Matched 7547 features.
# Writing MatrixEQTL Position file to: /data/work/example/pQTL/results_control/gene_locations.txt
# Done.
```
...
# -----------------------------------------------------------------------------
# [Step 1] Finding Common Samples...
# Found 49 common samples.
```
...
# Skipping low expression value filter (User requested).
# Missing Filter (NaN <= 20.0%): 7696 -> 7696 genes.
```
...
(Updated) 为了提高稳定性和灵活性，此步骤现已拆分为三个子步骤。

#### Step 4a: 基因型 PCA (Genotype PCA)
...
使用 R `peer` 包计算表达量隐藏因子（需安装 R `peer` 包）。
...
#### Step 4c: 合并协变量 & 绘制相关性 (Combine & Plot)

合并 PCA、PEER 和用户提供的已知协变量，并自动生成相关性热图 (`covariates_correlation.pdf`)。
...
如果您想单独检查任意协变量文件的相关性（而不运行合并步骤），可以使用此独立脚本。

```bash
python3 src/matrix_eqtl/utils/plot_correlation.py \
    --input results/covariates/final_covariates.txt \
    --output results/covariates/custom_heatmap.pdf
```

* **--input**: 输入的协变量文件 (Tab分隔，Rows=Covariates, Cols=Samples)。
* **--output**: (可选) 输出 PDF 路径。默认在输入文件同目录下生成 `_correlation.pdf`。

### Step 5: 执行 MatrixEQTL 分析 (Run Analysis)

最后运行 MatrixEQTL 核心分析。我们提供了两个便捷脚本：

#### 方式 A: 标准分析 (Cis + Trans)

使用 `examples/run_cis_trans_analysis.sh` 或以下命令：
...
#### 方式 B: 全两两分析 (All-Pairs / Location-Free)

不依赖位置文件，计算所有 Gene-SNP 对的相关性。使用 `examples/run_all_pairs_analysis.sh` 或：
...
* `--p-value`: 输出结果的 P 值阈值。
* `--trans-p-value`: Trans 结果阈值 (仅在标准模式下有效)。
* `--no-fdr`: 跳过 FDR 计算以节省内存。
